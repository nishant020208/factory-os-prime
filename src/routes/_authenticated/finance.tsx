...
const PURCHASE_FORM_FIELDS: FormField[] = [
  ...
  { key: "total_amount", label: "Amount ($)", type: "number", placeholder: "25000", required: true },
...
type FinanceRow = { id: string; po_number: string; total_amount: number | null; status: string; expected_date: string | null; created_at: string; _type: string };
...
function FinancePage() {
  ...
  const totalRevenue = allRecords.filter((r: any) => r.status === "received" || r.status === "completed").reduce((s, r) => s + Number(r.total_amount ?? 0), 0);
  const arOutstanding = allRecords.filter((r: any) => r.status === "pending" || r.status === "approved").reduce((s, r) => s + Number(r.total_amount ?? 0), 0);
  const apOutstanding = allRecords.filter((r: any) => r._type === "purchase" && r.status !== "received").reduce((s, r) => s + Number(r.total_amount ?? 0), 0);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["fin-pos"] }); toast.success("Purchase order created"); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["fin-pos"] }); toast.success("Purchase order updated"); },
    onError: (err: any) => toast.error(err.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["fin-pos"] }); toast.success("Purchase order deleted"); },
    onError: (err: any) => toast.error(err.message),
  });
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      moduleName="finance"
        ...
        kpis={
          <>
            <Kpi label="Revenue (Settled)" value={`$${(totalRevenue / 1000).toFixed(0)}k`} delta="+11.4%" icon={TrendingUp} tone="success" />
            <Kpi label="Outstanding" value={`$${(arOutstanding / 1000).toFixed(0)}k`} icon={Receipt} tone="warning" />
            <Kpi label="AP Pending" value={`$${(apOutstanding / 1000).toFixed(0)}k`} icon={Landmark} tone="info" />
            <Kpi label="Cash Position" value="$6.7M" delta="+3.1%" icon={PiggyBank} tone="primary" />
          </>
        }
        columns={[
          { key: "po_number", header: "Reference #", render: (r) => <span className="font-mono text-xs font-medium">{r.po_number}</span> },
          { key: "_type", header: "Type", render: (r) => <span className="text-xs capitalize">{r._type === "purchase" ? "AP" : "AR"}</span> },
          ...
          { key: "expected_date", header: "Date", hideOnMobile: true, render: (r) => r.expected_date ? new Date(r.expected_date).toLocaleDateString() : "—" },
        ]}
      />
      <div className="mt-4">
