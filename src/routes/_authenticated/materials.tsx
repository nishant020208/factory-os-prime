  ...
  head: () => ({
    meta: [
      { title: "Materials — FactoryOS AI" },
      {
        name: "description",
        content:
          "Material master list — single source for customer orders and production planning.",
      },
    ],
  }),
...
function MaterialsPage() {
  ...
  const { data: materials } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("materials").select("*").eq("company_id", companyId!).order("name"))
        .data ?? [],
    enabled: !!companyId,
  });
  ...
  const totalValue = (materials ?? []).reduce(
    (s: number, m: any) => s + Number(m.unit_cost ?? 0),
    0,
  );
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4">
        ...
        <Kpi
          label="Active"
          value={String((materials ?? []).filter((m: any) => m.is_active !== false).length)}
          icon={Package}
          tone="success"
        />
        <Kpi
      ...
      >
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle>{showEdit.open ? "Edit Material" : "New Material"}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => (showEdit.open ? updateMutation.mutate() : createMutation.mutate())}
              disabled={!form.name}
