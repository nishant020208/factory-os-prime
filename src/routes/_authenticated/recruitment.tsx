  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <Kpi
          label="In Interview"
          value={String(POSITIONS.filter((p) => p.status === "interviewing").length)}
          icon={Clock}
          tone="warning"
        />
        <Kpi
          label="Offers Made"
          value={String(POSITIONS.filter((p) => p.status === "offer").length)}
          icon={CheckCircle2}
          tone="success"
        />
      </div>
              ...
              <div key={p.id} className="flex items-center justify-between gap-3 py-3 px-2">
                <div>
                  ...
                  <div className="text-[11px] text-muted-foreground mt-0.5">
                    {p.dept} · {p.applicants} applicants · Posted {p.posted}
                  </div>
                </div>
