...
export const Route = createFileRoute("/_authenticated/suppliers")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Supplier management, performance tracking and procurement.",
      },
    ],
  }),
      ...
      kpis={
        <>
          <Kpi
            ...
            value={String(data?.filter((s) => s.status === "active").length ?? 0)}
            icon={Truck}
            tone="primary"
          />
          <Kpi
          ...
          <Kpi
            label="With Rating"
            value={String(data?.filter((s) => Number(s.rating ?? 0) > 0).length ?? 0)}
            icon={Clock}
            tone="warning"
          />
        </>
