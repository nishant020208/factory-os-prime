...
const PRODUCT_FORM_FIELDS: FormField[] = [
  ...
  { key: "name", label: "Product Name", type: "text", placeholder: "Titanium Bracket TB-600", required: true },
  { key: "description", label: "Description", type: "textarea", placeholder: "Product description" },
  { key: "unit", label: "Unit", type: "select", defaultValue: "pcs", options: [
...
function ProductsPage() {
  ...
  const totalValue = (data ?? []).reduce((s, p) => s + Number(p.unit_price ?? 0) * Number(p.reorder_level ?? 0), 0);
  ...
  return (
    <ResourceView
      ...
      kpis={
        <>
          <Kpi label="Active SKUs" value={String(activeCount)} delta="+3" icon={Package} tone="primary" />
          <Kpi label="Catalog Value" value={`$${(totalValue / 1000).toFixed(0)}k`} delta="+8.1%" icon={DollarSign} tone="success" />
          <Kpi label="Total SKUs" value={String(data?.length ?? 0)} icon={Layers} tone="info" />
          <Kpi label="Below Reorder" value={String(data?.filter(p => Number(p.reorder_level ?? 0) > 0).length ?? 0)} icon={Sparkles} tone="warning" />
        </>
      }
      columns={[
