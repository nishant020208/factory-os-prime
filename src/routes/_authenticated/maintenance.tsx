...
export const Route = createFileRoute("/_authenticated/maintenance")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Preventive, corrective and AI-driven predictive maintenance.",
      },
    ],
  }),
...
function MaintenancePage() {
  ...
  const down =
    machines?.filter((m) => m.status === "down" || m.status === "maintenance").length ?? 0;
  ...
  const resolvedBreaks = (breakdowns ?? []).filter(
    (b: any) => b.downtime_start && b.downtime_end,
  ) as Array<{ downtime_start: string; downtime_end: string }>;
  const mttrH = resolvedBreaks.length
    ? resolvedBreaks.reduce(
        (s, b) =>
          s + (new Date(b.downtime_end).getTime() - new Date(b.downtime_start).getTime()) / 3600000,
        0,
      ) / resolvedBreaks.length
    : null;
  ...
  const fmt = (h: number | null) =>
    h === null ? "—" : `${h >= 100 ? Math.round(h) : h.toFixed(1)} h`;
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <Kpi
          label="Operational"
          value={`${operational}/${machines?.length ?? 0}`}
          icon={Activity}
          tone="primary"
        />
        ...
        <Panel title="Maintenance Work Orders">
          {(tickets ?? []).length === 0 ? (
            <EmptyState
              ...
              sub="New tickets appear here. Create one with New Work Order."
            />
          ) : (
            <div className="divide-y divide-white/5">
              {(tickets ?? []).map((w: any) => (
                <div
                  key={w.id}
                  className="grid grid-cols-1 sm:grid-cols-[auto_1fr_auto_auto] items-center gap-2 sm:gap-3 py-3 text-sm"
                >
        ...
        <Panel title="Machine Status">
          {(machines ?? []).length === 0 ? (
            <EmptyState
              ...
              sub="Machine status appears here once machines are added."
            />
          ) : (
              ...
              {(machines ?? []).map((m) => (
                <div key={m.id} className="rounded-xl bg-card/60 border border-white/5 p-3">
                  ...
                  <div className="mt-2 h-1.5 rounded-full bg-white/5 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${Number(m.utilization) > 80 ? "bg-success" : Number(m.utilization) > 50 ? "bg-warning" : "bg-destructive"}`}
                      style={{ width: `${Math.min(Number(m.utilization ?? 0), 100)}%` }}
                    />
                  </div>
                  <div className="mt-1 text-[10px] text-muted-foreground text-right">
                    {Math.round(Number(m.utilization ?? 0))}% utilization
                  </div>
                </div>
              ))}
            </div>
