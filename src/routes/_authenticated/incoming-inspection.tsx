  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      <PageHeader
        ...
        sub="Inbound material quality inspection, acceptance and rejection tracking."
      />
        ...
        <Panel title="Recent Inspections">
          <div className="divide-y divide-white/5 text-sm">
            {(suppliers ?? []).slice(0, 5).map((s: any, i: number) => (
              <div
                key={s.id}
                className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] items-center gap-2 py-3 px-2"
              >
                <div>
                  <div className="font-medium">{s.name}</div>
                  <div className="text-[11px] text-muted-foreground">
                    PO received ·{" "}
                    {["2 days ago", "5 days ago", "1 week ago", "2 weeks ago", "3 weeks ago"][i]}
                  </div>
                </div>
