...
export const Route = createFileRoute("/_authenticated/attendance")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Employee attendance tracking, shift management and time clock." },
  ]}),
...
function AttendancePage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Attendance recorded"); setShowNew(false); queryClient.invalidateQueries({ queryKey: ["att-profiles"] }); },
  ...
  const records = (profiles ?? []).map((p: any, i: number) => ({
    ...
    check_out: i % 5 === 4 ? null : i % 3 === 0 ? null : `${4 + (i % 3)}:${String((i * 7) % 60).padStart(2, "0")} PM`,
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      <PageHeader eyebrow="People" title="Attendance" sub="Daily attendance tracking, check-in/out and shift management."
        actions={<Button className="bg-[image:var(--gradient-primary)] shadow-glow" onClick={() => setShowNew(true)}><Plus className="h-4 w-4 mr-1.5" />Record Attendance</Button>} />
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader><DialogTitle>Record Attendance</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1.5"><Label>Employee</Label><Select value={empId} onValueChange={setEmpId}><SelectTrigger><SelectValue placeholder="Select employee" /></SelectTrigger><SelectContent>{(profiles ?? []).map((p: any) => <SelectItem key={p.id} value={p.id}>{p.full_name ?? p.email}</SelectItem>)}</SelectContent></Select></div>
            <div className="space-y-1.5"><Label>Check In Time</Label><Input type="time" value={checkIn} onChange={e => setCheckIn(e.target.value)} /></div>
          </div>
          <DialogFooter><Button onClick={() => createMutation.mutate()} disabled={!empId || createMutation.isPending} className="bg-[image:var(--gradient-primary)]">Record</Button></DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="text-left text-[11px] uppercase tracking-wider text-muted-foreground py-2 px-2">Check In</th>
                  <th className="text-left text-[11px] uppercase tracking-wider text-muted-foreground py-2 px-2 hidden md:table-cell">Check Out</th>
                  <th className="text-left text-[11px] uppercase tracking-wider text-muted-foreground py-2 px-2">Hours</th>
