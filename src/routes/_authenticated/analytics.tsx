...
const COLORS = ["oklch(0.58 0.22 259)", "oklch(0.62 0.19 300)", "oklch(0.72 0.14 210)", "oklch(0.72 0.19 145)", "oklch(0.62 0.23 25)"];
...
function AnalyticsPage() {
  ...
  const avgProgress = totalOrders > 0 ? (prodOrders!.reduce((s: number, o: any) => s + Number(o.progress ?? 0), 0) / totalOrders) : 0;
  const machineUp = machines?.filter((m: any) => m.status === "operational").length ?? 0;
  const machineDown = machines?.filter((m: any) => m.status === "down" || m.status === "maintenance").length ?? 0;
  const totalPOValue = purchaseOrders?.reduce((s: number, p: any) => s + Number(p.total_amount ?? 0), 0) ?? 0;
  const totalInventory = inventory?.reduce((s: number, i: any) => s + Number(i.quantity ?? 0), 0) ?? 0;
  ...
  const machinePie = [
    ...
    { name: "Maintenance", value: machines?.filter((m: any) => m.status === "maintenance").length ?? 0 },
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
        ...
        actions={
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-primary bg-primary/10 border border-primary/20 rounded-full px-2 py-1 flex items-center gap-1">
              <BrainCircuit className="h-3 w-3" />AI Enhanced
            </span>
          </div>
        }
      />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Kpi label="Production Output" value={String(prodTrend[prodTrend.length - 1].output)} delta="+12.4%" icon={BarChart3} tone="primary" />
        <Kpi label="Completion Rate" value={`${avgProgress.toFixed(0)}%`} delta="+3.2%" icon={TrendingUp} tone="success" />
        <Kpi label="Machine Uptime" value={`${machines?.length ? ((machineUp / machines.length) * 100).toFixed(1) : 0}%`} icon={Activity} tone="info" />
        <Kpi label="Procurement Value" value={`$${(totalPOValue / 1000).toFixed(0)}k`} icon={PieChart} tone="warning" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart data={prodTrend}>
                ...
                <Area type="monotone" dataKey="orders" stroke="oklch(0.58 0.22 259)" fill="url(#a-o)" strokeWidth={2} name="Orders" />
                <Area type="monotone" dataKey="completed" stroke="oklch(0.72 0.19 145)" fill="url(#a-c)" strokeWidth={2} name="Completed" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Panel>
        ...
        <Panel title="Machine Status">
          <div className="h-64">
            <ResponsiveContainer>
              <RePie>
                <Pie data={machinePie} dataKey="value" innerRadius={50} outerRadius={80} paddingAngle={4} label={({ name, value }) => `${name}: ${value}`}>
                  {machinePie.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
                <Tooltip contentStyle={{ background: "oklch(0.20 0.025 260)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, fontSize: 12 }} />
              </RePie>
            </ResponsiveContainer>
              ...
              <BarChart data={machineUtil}>
                ...
                <Bar dataKey="utilization" radius={[4, 4, 0, 0]}>
                  {machineUtil.map((entry, i) => (
                    <Cell key={i} fill={entry.utilization > 80 ? "oklch(0.72 0.19 145)" : entry.utilization > 50 ? "oklch(0.75 0.18 80)" : "oklch(0.62 0.23 25)"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <LineChart data={revenueProjection}>
                ...
                <Tooltip contentStyle={{ background: "oklch(0.20 0.025 260)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, fontSize: 12 }} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                <Line type="monotone" dataKey="actual" stroke="oklch(0.58 0.22 259)" strokeWidth={2} dot={false} name="Actual" />
                <Line type="monotone" dataKey="projected" stroke="oklch(0.62 0.19 300)" strokeWidth={2} strokeDasharray="5 5" dot={false} name="Projected" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Panel>
      ...
      <div className="mt-4">
        <Panel title="AI Insights" right={<span className="text-[10px] text-primary flex items-center gap-1"><Zap className="h-3 w-3" />Live</span>}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { t: "Production output trending +12.4% month-over-month. Consider expanding Shift 2 capacity.", conf: 92, type: "success" },
              { t: `${machineDown} machine(s) currently down. Predictive maintenance window recommended within 48h.`, conf: 88, type: "warning" },
              { t: `${totalInventory.toLocaleString()} units in stock across all warehouses. Reorder trigger on 2 SKUs.`, conf: 95, type: "info" },
            ].map((insight, i) => (
              <div key={i} className="rounded-xl bg-card/60 border border-white/5 p-4">
                <div className="flex items-center justify-between text-[10px] text-primary mb-2">
                  <span className="flex items-center gap-1"><BrainCircuit className="h-3 w-3" /> AI Copilot</span>
