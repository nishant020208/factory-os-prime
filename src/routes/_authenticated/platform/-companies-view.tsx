  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <Kpi
          label="Active"
          value={String(data?.filter((c) => (c.status ?? "active") === "active").length ?? 0)}
          icon={ShieldCheck}
          tone="success"
        />
        <Kpi
          label="Pending"
          value={String(data?.filter((c) => c.status === "pending").length ?? 0)}
          icon={Building2}
          tone="warning"
        />
        <Kpi
          label="Suspended"
          value={String(data?.filter((c) => c.status === "suspended").length ?? 0)}
          icon={ClipboardX}
          tone="destructive"
        />
      </div>
              ...
              <div key={c.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 py-3">
                <div>
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    {c.industry ?? "—"} · {c.country ?? "—"} · {c.currency ?? "—"}
                  </div>
                </div>
                ...
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (confirm(`Delete ${c.name}? This cannot be undone.`)) del.mutate(c.id);
                    }}
                  >
