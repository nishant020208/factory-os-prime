...
import { Loader2, Save, Settings2, Globe, Shield, Bell, CreditCard, Database, Lock, Sliders } from "lucide-react";
...
const DEFAULT_SETTINGS: Record<string, PlatformSetting> = {
  invoice_qr_at_approval: { key: "invoice_qr_at_approval", value: true, description: "Auto-generate QR code on invoice at order approval" },
  auto_notify_next_role: { key: "auto_notify_next_role", value: true, description: "Auto-notify the next role at every status transition" },
  company_auto_approve: { key: "company_auto_approve", value: false, description: "Auto-approve new company registrations" },
  max_companies_per_admin: { key: "max_companies_per_admin", value: 5, description: "Maximum companies a Root Admin can manage" },
  audit_retention_days: { key: "audit_retention_days", value: 365, description: "Days to retain audit logs" },
  session_timeout_minutes: { key: "session_timeout_minutes", value: 480, description: "User session timeout in minutes" },
  allow_self_registration: { key: "allow_self_registration", value: true, description: "Allow users to self-register (whitelist required)" },
  require_mfa: { key: "require_mfa", value: false, description: "Require multi-factor authentication for all users" },
  default_currency: { key: "default_currency", value: "USD", description: "Default currency for new companies" },
  default_timezone: { key: "default_timezone", value: "America/New_York", description: "Default timezone for new companies" },
};
    ...
    <div className="max-w-[1000px] mx-auto space-y-4">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="mb-4">
          <TabsTrigger value="general"><Settings2 className="h-4 w-4 mr-1.5" />General</TabsTrigger>
          ...
          <TabsTrigger value="defaults"><Globe className="h-4 w-4 mr-1.5" />Defaults</TabsTrigger>
        </TabsList>
          ...
          <Panel title="General Settings">
            ...
            <div className="flex items-center justify-between py-3 border-b border-white/5">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{DEFAULT_SETTINGS.max_companies_per_admin.description}</div>
              </div>
              <Input
                type="number"
          ...
          <Panel title="Security Settings">
            ...
            <div className="flex items-center justify-between py-3 border-b border-white/5">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{DEFAULT_SETTINGS.session_timeout_minutes.description}</div>
              </div>
              <Select
            ...
            <div className="flex items-center justify-between py-3 border-b border-white/5">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{DEFAULT_SETTINGS.audit_retention_days.description}</div>
              </div>
              <Select
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">Used when creating new companies</div>
                </div>
                <Select
              ...
              <div className="flex items-center justify-between py-2">
                ...
                >
                  ...
                  <SelectContent>
                    {["America/New_York", "America/Chicago", "America/Los_Angeles", "Europe/London", "Asia/Kolkata", "Asia/Tokyo", "Asia/Singapore"].map(t => (
