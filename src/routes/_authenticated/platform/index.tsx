  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Kpi label="Total Companies" value={String(companies.data?.length ?? 0)} icon={Building2} tone="primary" />
          ...
          <Panel title="Recent tenants">
            <div className="divide-y divide-white/5 text-sm">
              {(companies.data ?? []).slice(0, 8).map(c => (
                <div key={c.id} className="grid grid-cols-[1fr_auto_auto] gap-3 py-2.5 items-center">
                  <div><div className="font-medium">{c.name}</div><div className="text-[11px] text-muted-foreground">{c.industry ?? "—"} · {c.country ?? "—"}</div></div>
                  <StatusBadge status={c.status ?? "active"} />
                  <div className="text-xs text-muted-foreground">{safeDate(c.created_at)}</div>
                </div>
              ))}
              {companies.isFetched && (companies.data ?? []).length === 0 && (
                <div className="py-6 text-xs text-muted-foreground">No companies yet — whitelist a Company Admin to onboard your first tenant.</div>
              )}
            </div>
          </Panel>
        </div>
        <Panel title="Recent whitelist activity" right={<FileCheck2 className="h-4 w-4 text-muted-foreground" />}>
          <div className="divide-y divide-white/5 text-sm">
            {(whitelist.data ?? []).slice(0, 6).map(w => (
              <div key={w.id} className="grid grid-cols-[1fr_auto] gap-3 py-2 items-center">
                <div className="truncate"><div className="font-medium truncate">{w.email}</div><div className="text-[11px] text-muted-foreground capitalize">{w.role.replace(/_/g," ")}</div></div>
      ...
      <div className="mt-4">
        <Panel title="Latest platform audit events" right={<ScrollText className="h-4 w-4 text-muted-foreground" />}>
          <div className="divide-y divide-white/5 text-sm">
            {(audit.data ?? []).map(l => (
