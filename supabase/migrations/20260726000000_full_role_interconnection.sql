-- =============================================================
-- FACTORYOS AI — FULL ROLE INTERCONNECTION & CUSTOMER ORDER WIRING
-- Phase: New tables, columns, RLS policies for complete end-to-end flow
-- =============================================================

-- ===================== NEW TABLES ============================

-- 1. Profile change requests (for universal Profile + Settings)
...
CREATE POLICY pcr_all ON public.profile_change_requests FOR ALL TO authenticated
...
-- 2. Order status history (tracks every customer order status transition)
...
CREATE POLICY osh_all ON public.order_status_history FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id())
  WITH CHECK (is_root_admin(auth.uid()) OR company_id = current_company_id());

-- 3. QR codes for invoices
...
CREATE POLICY qr_all ON public.qr_codes FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id())
  WITH CHECK (is_root_admin(auth.uid()) OR company_id = current_company_id());

-- 4. Company registration requests (for OAuth registration flow)
...
CREATE POLICY cr_all ON public.company_registrations FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR true)
  WITH CHECK (true);

-- 5. Platform settings (for Root Super Admin)
...
CREATE POLICY ps_select ON public.platform_settings FOR SELECT TO authenticated
  USING (true);
...
-- 6. Customer documents (uploaded files per customer)
...
CREATE POLICY cd_all ON public.customer_documents FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id())
  WITH CHECK (is_root_admin(auth.uid()) OR company_id = current_company_id());
...
CREATE POLICY ed_all ON public.employee_departments FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id())
  WITH CHECK (is_root_admin(auth.uid()) OR company_id = current_company_id());

-- 8. Inventory adjustment log (fixes Bug A by logging all adjustments)
...
CREATE POLICY ia_all ON public.inventory_adjustments FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id())
  WITH CHECK (is_root_admin(auth.uid()) OR company_id = current_company_id());

-- 9. Dashboard notes (fixes Bug B - AI/persistent notes)
...
CREATE POLICY dn_all ON public.dashboard_notes FOR ALL TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id())
  WITH CHECK (is_root_admin(auth.uid()) OR company_id = current_company_id());

-- ===================== COLUMN ADDITIONS ====================

-- Add sales_order_id to production_orders (links customer orders to production)
ALTER TABLE public.production_orders ADD COLUMN IF NOT EXISTS sales_order_id uuid;

-- Add qr_code_url to invoices
...
-- Add approved_by to sales_orders (tracks who approved)
...
-- Add email notifications preferences to companies
...
-- ===================== AUDITOR READ-ONLY POLICIES ===================

-- For all existing tables, add a second SELECT-only policy for Auditor.
-- This ensures Auditor can SELECT but never INSERT/UPDATE/DELETE.
-- Note: The existing "all" policies still apply for non-auditor roles.

-- Helper: check if the current user is an auditor
...
-- New tables with auditor read-only policies
-- profile_change_requests: auditor can't write
...
CREATE POLICY pcr_select ON public.profile_change_requests FOR SELECT TO authenticated
  USING (is_root_admin(auth.uid()) OR company_id = current_company_id());
CREATE POLICY pcr_all ON public.profile_change_requests FOR INSERT TO authenticated
  WITH CHECK ((is_root_admin(auth.uid()) OR company_id = current_company_id()) AND NOT is_auditor());
CREATE POLICY pcr_update ON public.profile_change_requests FOR UPDATE TO authenticated
  USING ((is_root_admin(auth.uid()) OR company_id = current_company_id()) AND NOT is_auditor())
  WITH CHECK ((is_root_admin(auth.uid()) OR company_id = current_company_id()) AND NOT is_auditor());

-- order_status_history: auditor can't write
...
-- qr_codes: auditor can't write
DROP POLICY IF EXISTS qr_all ON public.qr_codes;
...
-- company_registrations: auditor can't write
DROP POLICY IF EXISTS cr_all ON public.company_registrations;
...
CREATE POLICY cr_update ON public.company_registrations FOR UPDATE TO authenticated
  USING (is_root_admin(auth.uid()) AND NOT is_auditor());

-- dashboard_notes: auditor can't write
DROP POLICY IF EXISTS dn_all ON public.dashboard_notes;
...
-- inventory_adjustments: auditor can't write
DROP POLICY IF EXISTS ia_all ON public.inventory_adjustments;
...
-- customer_documents: auditor can't write
DROP POLICY IF EXISTS cd_all ON public.customer_documents;
...
-- employee_departments: auditor can't write
DROP POLICY IF EXISTS ed_all ON public.employee_departments;
...
-- Helper function to create notifications + audit logs in one call
...
BEGIN
  ...
  -- Insert audit log (skip if action is NULL, which means root action excluded)
...
-- Helper function to record status transition
