-- =====================================================================
-- FACTORYOS AI — PO RPC: expected date as text (2026-09-04)
--
-- The initial create_purchase_order_with_items RPC declared
-- p_expected_date DATE, which made the generated client type require a
-- string and rejected a NULL/empty JSON value at the PostgREST boundary.
-- This redefines the parameter as TEXT so the UI can pass an empty string
-- when no expected date was chosen; the function converts '' → NULL.
-- =====================================================================

-- Drop the previous overloads (DATE-parameter version from 00011, and any
-- partial TEXT-parameter attempt) before recreating with the TEXT parameter.
DROP FUNCTION IF EXISTS public.create_purchase_order_with_items(uuid, text, uuid, date, jsonb);
