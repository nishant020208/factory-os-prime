-- Operators may record completion state for the checklist supplied by their manager.
...
BEGIN
  IF EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'production_operator') THEN
    IF OLD.operator_id IS DISTINCT FROM auth.uid()
      ...
      OR NEW.assigned_at IS DISTINCT FROM OLD.assigned_at OR NEW.due_date IS DISTINCT FROM OLD.due_date THEN
